...
export function InvoiceEditor({ invoice, onClose, onSaved }: Readonly<{ invoice?: InvoiceItem; onClose: () => void; onSaved: () => void }>) {
  ...
  return <div aria-labelledby="invoice-editor-title" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-3 sm:p-4" role="dialog"><form className="max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-4 shadow-xl sm:max-h-[90dvh] sm:p-6" onSubmit={(event) => { event.preventDefault(); void save(); }}>
    <div className="flex items-start justify-between border-b border-[var(--border)] pb-4"><div><h2 className="text-lg font-bold" id="invoice-editor-title">{invoice ? "Cập nhật hóa đơn" : "Lập hóa đơn"}</h2><p className="mt-1 text-sm text-[var(--muted)]">Số tiền lấy từ booking; trạng thái hóa đơn đi theo trạng thái lưu trú.</p></div><Button onClick={onClose} variant="ghost">Đóng</Button></div>
    ...
    <div className="mt-5 grid gap-4 md:grid-cols-2">
      ...
      <div><p className="mb-1.5 text-sm font-semibold text-slate-700">Trạng thái</p><div className="flex min-h-10 items-center rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm font-medium">{form.status === "ISSUED" ? "Đã phát hành" : form.status === "VOID" ? "Đã hủy" : "Nháp"}</div></div>
