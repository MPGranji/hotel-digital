...
export function InvoiceEditor({ invoice, onClose, onSaved }: Readonly<{ invoice?: InvoiceItem; onClose: () => void; onSaved: () => void }>) {
  ...
  return <div aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog"><form className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-xl" onSubmit={(event) => { event.preventDefault(); void save(); }}>
    <div className="flex items-start justify-between border-b border-slate-200 pb-4"><div><h2 className="text-lg font-bold">{invoice ? "Cập nhật hóa đơn" : "Lập hóa đơn"}</h2><p className="mt-1 text-sm text-slate-500">Số tiền được chụp từ booking tại lúc lưu hóa đơn.</p></div><Button onClick={onClose} variant="ghost">Đóng</Button></div>
    {error ? <p className="mt-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p> : null}
    <div className="mt-5 grid gap-4 md:grid-cols-2">
      <div className="md:col-span-2"><Field error={fieldErrors.bookingId?.[0]} htmlFor="invoiceBooking" label="Booking" required><SearchableSelect disabled={Boolean(invoice)} id="invoiceBooking" onChange={(value) => setForm((x) => ({ ...x, bookingId: Number(value) }))} onSearchChange={setBookingSearch} options={bookingOptions} placeholder="Chọn booking" searchPlaceholder="Nhập mã booking, tên hoặc SĐT…" searchValue={bookingSearch} value={String(form.bookingId || "")} /></Field></div>
      <div><p className="mb-1.5 text-sm font-medium text-slate-700">Số hóa đơn</p><div className="flex min-h-10 items-center rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm font-semibold text-[var(--primary)]">{invoice?.invoiceNumber ?? "Hệ thống tự tạo khi lưu"}</div></div>
