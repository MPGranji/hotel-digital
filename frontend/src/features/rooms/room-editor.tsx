...
export function RoomEditor({ room, onClose, onSaved }: Readonly<{ room?: RoomListItem; onClose: () => void; onSaved: () => void }>) {
  ...
  return <div aria-labelledby="room-editor-title" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-3 sm:p-4" role="dialog">
    <form className="max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-4 shadow-xl sm:max-h-[90dvh] sm:p-6" onSubmit={(event) => { event.preventDefault(); void save(); }}>
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-4"><div><h2 className="text-lg font-semibold" id="room-editor-title">{room ? "Cập nhật phòng" : "Thêm phòng"}</h2><p className="mt-1 text-sm text-slate-500">Phòng đã có lịch sử đặt phòng có thể ngừng sử dụng khi cần.</p></div><Button onClick={onClose} variant="ghost">Đóng</Button></div>
