...
export function RoomBlockEditor({ blockId, initialRoomId, initialDate, rooms, onClose, onSaved }: Readonly<{ blockId?: number; initialRoomId?: number; initialDate?: string; rooms: RoomListItem[]; onClose: () => void; onSaved: () => void }>) {
  ...
  return <div aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog"><form className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-xl" onSubmit={(e) => { e.preventDefault(); void save(); }}>
    <div className="flex items-start justify-between border-b border-slate-200 pb-4"><div><h2 className="text-lg font-bold">{blockId ? "Cập nhật lịch bảo trì" : "Thêm lịch bảo trì"}</h2><p className="mt-1 text-sm text-slate-500">Phòng sẽ không xuất hiện trong danh sách phòng trống trong khoảng này.</p></div><Button onClick={onClose} variant="ghost">Đóng</Button></div>
    ...
    <div className="mt-6 flex justify-between gap-2 border-t border-slate-200 pt-4"><div>{blockId && form.isActive ? <Button disabled={saving} onClick={() => void save(false)} variant="danger">Hủy lịch bảo trì</Button> : null}</div><div className="flex gap-2"><Button onClick={onClose} variant="secondary">Đóng</Button><Button disabled={loading || saving || !form.isActive} type="submit">{saving ? "Đang lưu…" : "Lưu lịch"}</Button></div></div>
  </form></div>;
}
