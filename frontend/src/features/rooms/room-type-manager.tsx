...
export function RoomTypeManager({ onClose, onSaved }: Readonly<{ onClose: () => void; onSaved: () => void }>) {
  ...
  return <div aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog"><div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl bg-white p-6 shadow-xl">
    <div className="flex items-start justify-between border-b border-slate-200 pb-4"><div><h2 className="text-lg font-bold">Quản lý hạng phòng</h2><p className="mt-1 text-sm text-slate-500">Sức chứa là số khách tối đa; giá là giá niêm yết mặc định.</p></div><Button onClick={onClose} variant="ghost">Đóng</Button></div>
    ...
    <form className="mt-5 grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 md:grid-cols-5" onSubmit={(e) => { e.preventDefault(); void save(); }}>
      ...
      <Field error={fieldErrors.listedPricePerNight?.[0]} htmlFor="typePrice" label="Giá / đêm"><Input id="typePrice" min={0} onChange={(e) => setForm((x) => ({ ...x, listedPricePerNight: e.target.value ? Number(e.target.value) : undefined }))} type="number" value={form.listedPricePerNight ?? ""} /></Field>
    ...
    <div className="mt-5 overflow-x-auto rounded-lg border border-slate-200"><table className="w-full min-w-[650px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Mã / Tên</th><th className="px-4 py-3">Sức chứa</th><th className="px-4 py-3 text-right">Giá</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3 text-right">Phòng</th><th className="px-4 py-3"></th></tr></thead><tbody className="divide-y divide-slate-100">{items.map((item) => <tr key={item.id}><td className="px-4 py-3"><b>{item.code}</b><p className="text-xs text-slate-500">{item.name}</p></td><td className="px-4 py-3">{item.capacity} người</td><td className="px-4 py-3 text-right">{item.listedPricePerNight == null ? "—" : formatCurrency(item.listedPricePerNight)}</td><td className="px-4 py-3">{item.isActive ? "Hoạt động" : "Ngừng dùng"}</td><td className="px-4 py-3 text-right">{item.roomCount}</td><td className="px-4 py-3 text-right"><Button onClick={() => edit(item)} variant="secondary">Sửa</Button></td></tr>)}</tbody></table></div>
  </div></div>;
}
