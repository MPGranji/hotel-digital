...
export function PaymentSection({ model }: Readonly<{ model: FormModel }>) {
  ...
  const settledAmount = currentBooking.paidAmount + currentBooking.debtAmount;
  const amountToCollect = Math.max(currentBooking.grossRevenue - settledAmount, 0);
  ...
  async function save() {
    ...
    try {
      ...
      setItems(payments);
      setAmount(refreshed ? String(Math.max(refreshed.grossRevenue - refreshed.paidAmount - refreshed.debtAmount, 0)) : "");
      setReferenceCode("");
    } catch (reason) {
  ...
  return (
    <div>
      <SectionTitle>4. Thanh toán</SectionTitle>
      <div className="flex flex-col gap-2 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm sm:flex-row sm:items-center sm:divide-x sm:divide-slate-300">
        <MoneyValue label="Tổng tiền" value={currentBooking.grossRevenue} valueClass="text-[var(--primary)]" />
        ...
        <form className="mt-4 grid gap-4 rounded-xl border border-slate-200 bg-slate-50/70 p-4 md:grid-cols-[minmax(180px,1fr)_minmax(220px,1fr)_minmax(220px,1fr)_auto] md:items-end" onSubmit={(event) => { event.preventDefault(); void save(); }}>
          ...
          <Field error={fieldErrors.method?.[0]} htmlFor="paymentMethodRecord" label="Phương thức" required>
            <Select id="paymentMethodRecord" onChange={(event) => setMethod(event.target.value as PaymentMethod)} value={method}>
              ...
              <option value="TRANSFER">Chuyển khoản / online</option>
              <option value="CARD">Thẻ</option>
            </Select>
          </Field>
          <Field error={fieldErrors.referenceCode?.[0]} htmlFor="paymentReference" label="Mã giao dịch">
          ...
          <Button disabled={saving} type="submit">{saving ? "Đang lưu…" : "Ghi nhận thu"}</Button>
        </form>
      ) : null}

      {error ? <p className="mt-3 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p> : null}
      <div className="mt-4">
        {loading ? <DataMessage title="Đang tải thanh toán…" /> : items.length === 0 ? <DataMessage description="Có thể ghi nhận tiền cọc hoặc thu tiền khi khách trả phòng." title="Chưa có giao dịch thanh toán" /> : (
