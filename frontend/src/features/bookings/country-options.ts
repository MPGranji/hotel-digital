...
export const VIETNAM_NATIONALITY = "Vietnam";

export const countryOptions = countryCodes
  .map((code) => ({
    value: englishRegionNames.of(code) ?? code,
    label: englishRegionNames.of(code) ?? code,
    searchText: code,
  }))
  .sort((left, right) => {
    if (left.value === VIETNAM_NATIONALITY) return -1;
    if (right.value === VIETNAM_NATIONALITY) return 1;
