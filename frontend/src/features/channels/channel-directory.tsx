    ...
    <>
      ...
      <Panel>
        ...
        {error ? <DataMessage action={<Button onClick={refresh}>Thử lại</Button>} description={error} title="Không thể tải dữ liệu" /> : loading ? <DataMessage title="Đang tải danh sách kênh…" /> : channels.length === 0 ? <DataMessage description="Thử thay đổi bộ lọc hoặc thêm kênh mới." title="Không có kênh phù hợp" /> : (
          <div className="overflow-x-auto rounded-lg border border-slate-200"><table className="min-w-[760px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Mã kênh</th><th className="px-4 py-3">Tên kênh</th><th className="px-4 py-3">Nhóm</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3 text-right">Booking</th><th className="px-4 py-3 text-right">Thao tác</th></tr></thead><tbody className="divide-y divide-slate-100">{channels.map((channel) => <tr className="hover:bg-slate-50" key={channel.id}><td className="px-4 py-3 font-semibold text-[var(--primary)]">{channel.code}</td><td className="px-4 py-3"><p className="font-medium">{channel.name}</p>{channel.note ? <p className="max-w-md truncate text-xs text-slate-500">{channel.note}</p> : null}</td><td className="px-4 py-3"><span className="rounded-md border border-blue-100 bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">{categoryLabels[channel.category]}</span></td><td className="px-4 py-3"><span className={`rounded-md border px-2 py-1 text-xs font-semibold ${channel.isActive ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-100 text-slate-600"}`}>{channel.isActive ? "Đang hoạt động" : "Ngừng hoạt động"}</span></td><td className="px-4 py-3 text-right">{channel.bookingCount}</td><td className="px-4 py-3 text-right"><Button onClick={() => setEditing(channel)} variant="secondary">Sửa</Button></td></tr>)}</tbody></table></div>
        )}
      </Panel>
