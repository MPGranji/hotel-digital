...
export function ChannelDirectory() {
  ...
  return (
    <>
      <PageHeader actions={<Button onClick={() => setEditing("new")}>Thêm kênh</Button>} description="Quản lý kênh đặt phòng theo ba nhóm: Offline, Online và Đại lý du lịch." title="Kênh đặt phòng" />
      <Panel>
        <div className="mb-5 grid gap-3 border-b border-slate-200 pb-5 md:grid-cols-2 xl:grid-cols-[minmax(260px,1fr)_220px_220px_auto]">
          ...
          <Select aria-label="Nhóm kênh" onChange={(event) => { setCategory(event.target.value); setLoading(true); }} value={category}><option value="">Tất cả nhóm</option><option value="OFFLINE">Offline</option><option value="ONLINE">Online</option><option value="TRAVEL_AGENCY">Đại lý du lịch</option></Select>
